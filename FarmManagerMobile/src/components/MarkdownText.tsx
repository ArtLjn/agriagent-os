...
const tableRules = {
  table: (node: any, children: any, parent: any, styles: any) => (
    <ScrollView
      ...
      <View style={[styles.table, {alignSelf: 'flex-start'}]}>{children}</View>
    </ScrollView>
  ),
};
...
const h = (size: number, weight: string, color: string) => ({
  fontSize: size,
  fontWeight: weight as 'normal' | 'bold' | '100' | '200' | '300' | '400' | '500' | '600' | '700' | '800' | '900',
  color,
...
const styles = {
  ...
  bullet_list_icon: {...textBase, color: colors.primary, marginRight: spacing.sm},
  ordered_list_icon: {color: colors.primary, fontSize: fontSize.sm, fontWeight: '700' as const, lineHeight: 22, marginRight: spacing.sm, minWidth: 20},
  list_item: textBase,
  blockquote: {
    borderLeftWidth: 3,
