...
const STATUS_CONFIG: Record<string, {label: string; color: string; icon: string; bgColor: string}> = {
  active: {label: '进行中', color: colors.success, icon: 'play-circle', bgColor: colors.successLight},
  completed: {label: '已完成', color: colors.info, icon: 'check-circle', bgColor: colors.infoLight},
  abandoned: {label: '已废弃', color: colors.danger, icon: 'close-circle', bgColor: colors.dangerLight},
};
...
export const CycleListScreen: React.FC = () => {
  ...
  return (
    <View style={styles.container}>
                ...
                <View style={styles.body}>
                  ...
                  {item.current_stage_name && (
                    <View style={styles.stageRow}>
                      <Icon name="progress-clock" size={14} color={colors.primary} />
      ...
      />
      <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('CycleCreate')}>
