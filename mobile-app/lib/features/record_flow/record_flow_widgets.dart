...
class HeroUnderstandingCard extends StatelessWidget {
  const HeroUnderstandingCard({super.key});
  ...
  Widget build(BuildContext context) {
    return CardPanel(
      ...
      child: Stack(
        children: [
          const Positioned(right: -8, top: 12, child: _ReceiptGlow()),
          Column(
...
class _ReceiptGlow extends StatelessWidget {
  const _ReceiptGlow();
...
class OriginalTextCard extends StatelessWidget {
  const OriginalTextCard({super.key});
...
class SegmentedRecordType extends StatelessWidget {
  const SegmentedRecordType({super.key});
    ...
    return CardPanel(
      ...
      child: Row(
        children: const [
          ...
          SizedBox(width: 8),
          Expanded(child: SegmentItem(label: '农事', icon: LucideIcons.leaf)),
          SizedBox(width: 8),
          Expanded(
              child: SegmentItem(label: '工资', icon: LucideIcons.usersRound)),
        ],
      ),
    );
  }
...
class AmountCategoryEditor extends StatelessWidget {
  const AmountCategoryEditor({super.key});
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final categoryWidth = constraints.maxWidth >= 338 ? 166.0 : 148.0;
        ...
        return Row(
          ...
          children: [
            ...
            SizedBox(
              width: categoryWidth,
              child: const _CategorySelectTile(),
            ),
          ],
        );
      },
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: const [
          Expanded(
            child: Icon(LucideIcons.minus, size: 24, color: AppColors.blue),
          ),
          SizedBox(
            height: 22,
          ...
          Expanded(
            child: Icon(LucideIcons.plus, size: 24, color: AppColors.blue),
          ),
        ],
      ),
    );
...
class EditablePillRow extends StatelessWidget {
  const EditablePillRow({
    super.key,
    required this.label,
    required this.icon,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(LucideIcons.check, color: AppColors.greenDark, size: 16),
          const SizedBox(width: 5),
          Text(
            '已入账',
            style: AppTextStyles.sectionTitle.copyWith(
