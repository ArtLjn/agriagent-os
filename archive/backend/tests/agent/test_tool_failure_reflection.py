...
def test_reflect_tool_failure_repairs_missing_category_from_dynamic_candidates(
    ...
    assert decision.metadata["tool_failure_repair"]["strategy"] == "dynamic_exact_match"


def test_reflect_tool_failure_consolidates_item_to_existing_broad_category(
    monkeypatch,
):
    monkeypatch.setattr(
        "app.agent.executor.tool_failure_reflection.load_skill_candidates",
        lambda _farm_id: SimpleNamespace(values={"category": ["农资", "其他"]}),
    )
    ...
    assert decision.action == "ask_repaired_confirmation"
    assert decision.repaired_params["category"] == "农资"
    assert "农资" in decision.confirmation_text
    assert (
        decision.metadata["tool_failure_repair"]["strategy"] == "dynamic_consolidation"
    )
...
def test_reflect_tool_failure_repairs_missing_category_to_other(monkeypatch):
    ...
    assert decision.metadata["tool_failure_repair"]["strategy"] == "fallback_other"
