...
def test_policy_shortlists_model_choice_read_pool_in_legacy_mode() -> None:
    """legacy_recall_mode=true 时,RouterPolicy 走原 trim_candidates_by_budget 召回预选。"""
...
def test_policy_naive_mode_injects_all_read_tools_without_shortlist() -> None:
    """朴素模式下,RouterPolicy 全量注入所有 read skill,不做 trim_candidates_by_budget 预选。"""
    candidates = [_candidate(f"read_{index}", "read") for index in range(4)]
    ...
    # 朴素模式忽略 max_tools_default 限制,注入全部 read skill
