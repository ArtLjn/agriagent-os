...
def test_disclosure_budget_defaults_match_spec() -> None:
    """朴素全量注入模式默认预算(详见 agent-harness-design.md §5.0)。

    - max_tools_default=30: 允许全部 ~30 个 read skill 进入 bind_tools
    - max_schema_tokens=9000: 全量 schema 的硬上限保护(原 1800 是召回预算)
    """
    budget = DisclosureBudget()

    assert budget.max_tools_default == 30
    assert budget.max_tools_complex == 5
    assert budget.max_write_tools == 1
    assert budget.max_schema_tokens == 9000
