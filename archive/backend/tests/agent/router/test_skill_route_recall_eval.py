...
def test_retrievable_debt_read_uses_legacy_recall_in_legacy_mode() -> None:
    """legacy_recall_mode=true 时,"还有没结清的钱吗" 走原 BM25+向量召回。"""
    tools = [
        _tool("get_farm_status"),
        _tool("manage_cost"),
        _tool("manage_crop_cycle"),
    ...
    decision = SkillRouter(legacy_recall_mode=True).route("还有没结清的钱吗", tools)

    assert decision.selected_tools == ["manage_cost"]
    assert decision.selected_operations == {"manage_cost": ["query_debt"]}


def test_retrievable_debt_read_uses_fulltool_injection_in_naive_mode() -> None:
    """朴素模式下,"还有没结清的钱吗" 走全量注入,LLM 看到所有 read skill 自选。
