...
def _add_conversation_columns(inspector) -> None:
    ...
    if "last_turn_id" not in columns:
        op.add_column(
            "conversations", sa.Column("last_turn_id", sa.Integer(), nullable=True)
        )
    if "last_event_seq" not in columns:
        op.add_column(
            "conversations", sa.Column("last_event_seq", sa.Integer(), nullable=True)
        )
...
def _add_message_columns(inspector) -> None:
    ...
    if "turn_id" not in columns:
        op.add_column(
            "conversation_messages", sa.Column("turn_id", sa.Integer(), nullable=True)
        )
    if "ix_conversation_messages_turn_id" not in indexes:
        op.create_index(
            "ix_conversation_messages_turn_id", "conversation_messages", ["turn_id"]
        )
    ...
    if "meta_json" not in columns:
        op.add_column(
            "conversation_messages", sa.Column("meta_json", sa.JSON(), nullable=True)
        )
...
def _drop_message_columns(inspector) -> None:
    ...
    if "ix_conversation_messages_turn_id" in indexes:
        op.drop_index(
            "ix_conversation_messages_turn_id", table_name="conversation_messages"
        )
...
def _create_agent_turns(inspector) -> None:
    ...
    op.create_index(
        "ix_agent_turns_pending_plan_id", "agent_turns", ["pending_plan_id"]
    )
...
def _create_pending_plan_tables(inspector) -> None:
    ...
    if "agent_pending_plans" not in tables:
        ...
        op.create_index(
            "ix_agent_pending_plans_farm_id", "agent_pending_plans", ["farm_id"]
        )
        op.create_index(
            "ix_agent_pending_plans_session_id", "agent_pending_plans", ["session_id"]
        )
        op.create_index(
            "ix_agent_pending_plans_status", "agent_pending_plans", ["status"]
        )
        op.create_index(
            "ix_agent_pending_plans_expires_at", "agent_pending_plans", ["expires_at"]
        )
    else:
    ...
    if "agent_pending_plan_steps" not in tables:
        ...
        op.create_index(
            "ix_agent_pending_plan_steps_skill_name",
            "agent_pending_plan_steps",
            ["skill_name"],
        )
        op.create_index(
            "ix_agent_pending_plan_steps_status", "agent_pending_plan_steps", ["status"]
        )
    else:
...
def _upgrade_existing_pending_plans(inspector) -> None:
    ...
    if "router_decision_json" not in columns:
        ...
        if "router_decision" in columns:
            op.execute(
                "UPDATE agent_pending_plans SET router_decision_json = router_decision"
            )
...
def _upgrade_existing_pending_plan_steps(inspector) -> None:
    ...
    indexes = {
        index["name"] for index in inspector.get_indexes("agent_pending_plan_steps")
    }
    ...
    if "result_json" not in columns:
        ...
        if "result_payload" in columns:
            op.execute(
                "UPDATE agent_pending_plan_steps SET result_json = result_payload"
            )
    if "error_message" not in columns:
        ...
        if "error_payload" in columns:
            op.execute(
                "UPDATE agent_pending_plan_steps SET error_message = CAST(error_payload AS CHAR)"
            )
    indexes = {
        index["name"] for index in inspector.get_indexes("agent_pending_plan_steps")
    }
