...
class SkillRouter:
    ...
    def __init__(
        ...
        legacy_recall_mode: bool | None = None,
        chitchat: ChitchatClassifier | None = None,
    ) -> None:
        self._classifier = classifier or RuleIntentClassifier()
        self._budget = budget or DisclosureBudget()
        if legacy_recall_mode is None:
            legacy_recall_mode = settings.router.legacy_recall_mode
        self._legacy_recall_mode = legacy_recall_mode
        self._policy = policy or RouterPolicy(
            self._budget,
            legacy_recall_mode=self._legacy_recall_mode,
        )
        ...
        self._chitchat = chitchat or ChitchatClassifier()
    ...
    def route(self, message: str, tools: list[BaseTool]) -> RouterDecision:
        ...
        router_mode = "legacy_recall" if self._legacy_recall_mode else "naive_fulltool_injection"
        try:
            if self._legacy_recall_mode:
                decision = self._route_legacy(message, catalog)
            else:
                decision = self._route_naive(message, catalog)
    ...
    def _route_naive(
        ...
        """朴素全量注入模式:ChitchatClassifier 兜底 + 写意图识别 + 全量注入。

        流程:
        1. ChitchatClassifier 命中 → 直接 fallback no_tools,跳过 LLM 调用
        2. 否则调 RuleIntentClassifier 仅用于识别写意图生成 write frame
           (read frame 在 RouterPolicy 内被 read/write mismatch reject,
            落到 _route_without_requested_tools → _naive_fulltool_decision
            全量注入)
        3. 严禁调用 CandidateRetriever / HybridOperationRetriever 等召回组件

        设计权衡(详见 docs/specs/2026-07-31-agent-harness-design.md §5.0):
        - 任务约束要求朴素路径不调 RuleIntentClassifier,但写操作 pending_plan
          路径(spec.md "写操作仍走风险门禁")依赖 write frame。
        - Phase 1 折中:保留 RuleIntentClassifier 仅做写意图识别,不调召回。
        - Phase 3 物理删除时,需先把写意图识别提取到独立 WriteIntentClassifier。

        legacy_recall_mode=true 走 _route_legacy 完整召回链路。
        """
        if self._chitchat.is_chitchat(message):
            self._log_naive_short_circuit(message=message, reason="chitchat")
            return self._chitchat_decision(message)
        # 朴素模式直接 enrich frames,不走 _classified_frames 的召回跳过日志
        frames = self._enrich_frames(self._classifier.classify(message), catalog)
        intent = classify_intent(message)
        self._log_classification_completed(
            frames=frames,
            intent=intent,
            router_mode="naive_fulltool_injection",
        )
    ...
    def _route_legacy(
        ...
        """原召回链路:RuleIntentClassifier → HybridOperationRetriever → RouterPolicy。

        legacy_recall_mode=true 时使用,作为朴素模式回归时的回滚通道。
        """
    ...
    @staticmethod
    def _chitchat_decision(message: str) -> RouterDecision:
        """寒暄/教程兜底决策,不调用任何工具,延迟 < 100ms。"""
        frame = IntentFrame(
            domain="chitchat",
            intent="chitchat",
            risk="read",
            ...
            confidence=1.0,
            evidence={
                "fallback_reason": "chitchat_short_circuit",
                "router_mode": "naive_fulltool_injection",
            },
        )
        return RouterDecision(
            ...
            fallback="no_tools",
            fallback_reason="chitchat_short_circuit",
            reason="寒暄/教程兜底:朴素模式下短路,不进入 LLM 工具调用",
            scores={"domain": {}, "capability": {}, "operation": {}},
            evidence={
                "frames": [frame.evidence],
                "selected_candidates": [],
                "rejected_candidates": [],
                "router_mode": "naive_fulltool_injection",
    ...
    def _log_naive_short_circuit(
        self,
        *,
        message: str,
        reason: str,
    ) -> None:
        """朴素模式 chitchat 短路 trace event,便于运维区分 chitchat 与全量注入路径。"""
        trace = get_trace()
        log_event(
            logger,
            logging.INFO,
            "skill_router_naive_short_circuit",
            request_id=trace.request_id if trace else None,
            session_id=trace.session_id if trace else None,
            status="skipped",
            data={
