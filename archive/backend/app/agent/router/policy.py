...
class RouterPolicy:
    ...
    def _route_without_requested_tools(
        ...
        if not self._legacy_recall_mode:
            return self._naive_fulltool_decision(message, candidate_by_name)
    ...
    def _naive_fulltool_decision(
        self,
        message: str,
        candidate_by_name: dict[str, ToolCandidate],
    ) -> RouterDecision:
        """朴素模式:全量注入所有通过门禁的 read skill,不预过滤。

        详见 docs/specs/2026-07-31-agent-harness-design.md §5.0。
        - 所有 risk=read 且 enabled 的 skill 全部进 bind_tools
        - schema token 超 max_schema_tokens 时记 warning(不静默截断)
        - "重试/继续/不对"等上下文短输入同样进入此分支,LLM 借助 chat history 自选
        """
        selected = [
            candidate
        ...
        if total_tokens > self._budget.max_schema_tokens:
            logger.warning(
                "schema_token_budget_exceeded_naive total=%d budget=%d tool_count=%d "
                "message_preview=%r",
                total_tokens,
        ...
        frame = IntentFrame(
            domain="general",
            intent="naive_fulltool_read",
            risk="read",
            entities=["query"],
            candidate_tools=[],
            confidence=0.55,
            evidence={
                "fallback_reason": "naive_fulltool_injection",
                "candidate_count": len(selected),
                "schema_token_total": total_tokens,
                "budget_max_schema_tokens": self._budget.max_schema_tokens,
        ...
        return RouterDecision(
            ...
            fallback_reason="naive_fulltool_injection",
            reason="朴素模式:全量注入通过门禁的 read skill 交给 LLM 自选",
            schema_token_estimate=total_tokens,
