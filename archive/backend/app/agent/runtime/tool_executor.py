...
def release_session_lock(session_id: str | None) -> None:
    if session_id:
        lock = _session_locks.get(session_id)
...
async def _parallel_tool_node(state: AgentState) -> dict:
    ...
    if len(tool_calls) == 1:
        results = [
            await _call_one_with_session_lock(
                tc=tool_calls[0],
                tool_map=tool_map,
                state=state,
                farm_id=farm_id,
...
async def _call_one_with_session_lock(
    *,
    tc: dict,
    tool_map: dict,
    state: AgentState,
    ...
    async with _get_session_lock(session_id):
        lock_wait_ms = int((_time.perf_counter() - wait_started_at) * 1000)
        log_event(
            logger,
            logging.INFO,
            "tool_session_lock_acquired",
            step_id=f"tool-call-{tc['id']}",
            session_id=session_id,
            status="acquired",
            data={
                "tool": tc["name"],
...
__all__ = ["_parallel_tool_node", "release_session_lock"]
