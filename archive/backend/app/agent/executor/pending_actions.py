...
from app.agent.executor.pending_plan_bindings import resolve_step_param_bindings
from app.agent.executor.pending_plan_saga import (
    ExecutedPlanStep,
    compensate_executed_plan_steps,
    merge_pending_plan_failure_decision,
    pending_plan_compensation_block,
    pending_plan_failure_decision,
)
...
async def _confirm_pending_plan(
    ...
    if reflection_block is not None:
        return reflection_block
    compensation_block = pending_plan_compensation_block(plan)
    if compensation_block is not None:
        _cancel_pending_plan(farm_id, session_id)
        return compensation_block
    try:
        _confirm_pending_plan_in_db(plan.plan_id, plan.version, session_id=session_id)
    ...
    for step in sorted(plan.steps, key=lambda item: item.step_index):
        try:
            ...
            logger.exception(
                "pending plan step 执行异常 | plan=%s step=%s",
                plan.plan_id,
                step.step_index,
            )
            compensation = await compensate_executed_plan_steps(
                farm_id=farm_id,
                plan=plan,
                ...
                execute_write_skill=_execute_write_skill,
            )
            _clear_runtime_pending_plan_cache(farm_id, session_id)
            return pending_plan_failure_decision(
                step=step,
                result_reply=str(exc),
        ...
        if step_decision is not None:
            compensation = await compensate_executed_plan_steps(
                ...
                session_id=session_id,
                failed_step=step,
                failure_message=step_decision.reply,
                execute_write_skill=_execute_write_skill,
            )
            return merge_pending_plan_failure_decision(
...
async def _execute_pending_plan_step(
    *,
    ...
    try:
        params = resolve_step_param_bindings(params, results_by_step)
    ...
    if contract_block is not None:
        return contract_block, None, None, None, params
    ...
    if _skill_result_failed_or_needs_clarify(result):
        ...
        return decision, None, None, None, params
...
def _confirm_pending_plan_in_db(
    plan_id: str,
...
def _release_session_lock(session_id: str | None) -> None:
    if not session_id:
        return
    from app.agent.runtime.tool_executor import release_session_lock
