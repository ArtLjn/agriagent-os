...
def _start_skill_vector_sync_task() -> asyncio.Task[None] | None:
    ...
    朴素路由模式(settings.router.legacy_recall_mode=false)下不启动同步任务,
    因为 router 不调用向量索引(详见 docs/specs/2026-07-31-agent-harness-design.md §5.0)。
    切回 legacy 模式时,首次启动会重新同步。
    """
    if not (
        settings.skill_vector_store.enabled
        and settings.skill_vector_store.sync_on_startup
        ...
        return None
    if not settings.router.legacy_recall_mode:
        logger.info(
            "Skill 向量同步已跳过 | reason=naive_fulltool_injection "
