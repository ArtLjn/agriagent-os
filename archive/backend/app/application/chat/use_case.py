...
def _validate_chat_farm(farm: Farm) -> None:
    farm_id = getattr(farm, "id", None)
    if not isinstance(farm_id, int) or farm_id <= 0:
        raise HTTPException(
            status_code=400,
            detail={"code": "FARM_ID_MISSING", "detail": "农场标识缺失或无效"},
        )
...
async def chat(
    ...
    if decision.handled:
        if decision.metadata.get("http_status_code") == 409:
            raise HTTPException(
                status_code=409,
                detail={
                    "code": decision.metadata.get("code", "PENDING_PLAN_CONFLICT"),
                    "detail": reply_or_default(
                        decision.reply,
                        "操作正在处理中，请稍后再试",
                    ),
                },
...
def reply_or_default(reply: str, default: str) -> str:
    """返回可展示的回复文本。"""
