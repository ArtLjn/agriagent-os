...
class RouterConfig(BaseModel):
    ...
    legacy_recall_mode:
        true = 走原"召回 + LLM 自选"三层协作链路(RuleIntentClassifier /
               CandidateRetriever / HybridOperationRetriever / tool_selector)。
               生产发现朴素模式回归时切回,作为回滚通道。
        false (默认) = 朴素全量注入模式,直接 bind_tools(filtered_all_skills)
               让 LLM 自选,跳过召回链路。
...
class AgentConfig(BaseModel):
    strict_farm_id: bool = True


class PendingPlanConfig(BaseModel):
    concurrency_safety: bool = True
...
class Settings(BaseSettings):
    ...
    pending_plan: PendingPlanConfig = PendingPlanConfig()
