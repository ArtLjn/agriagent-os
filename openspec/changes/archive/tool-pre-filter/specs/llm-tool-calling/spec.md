...
- **WHEN** 用户发送"明天天气怎么样"且 `select_tools` 返回 `[weather]`
- **THEN** `bind_tools()` 仅注入 1 个 Tool，LLM 返回 `tool_calls` 包含 `weather`
...
- **WHEN** 用户发送"看看天气和成本"且 `select_tools` 返回 `[weather, get_cost_summary]`
