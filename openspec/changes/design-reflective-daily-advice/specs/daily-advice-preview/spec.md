...
#### Scenario: 不二次请求
- **WHEN** 详情页由首页点击进入
- **THEN** 详情页 SHALL 直接使用传入 item 的 `detail` 渲染，不调用 `/agent/daily/items/{id}`
...
#### Scenario: 展示单条建议详情
- **WHEN** 详情页收到一个包含 `detail` 的 item
- **THEN** 顶部卡片使用 `detail.title`、`detail.description` 和 `detail.hero_badges`，判断依据使用 `detail.evidence`，执行步骤使用 `detail.steps`

#### Scenario: 关联事项为空
- **WHEN** `detail.related` 为空数组
- **THEN** 详情页不显示空白关联事项列表，或显示“暂无关联事项”的轻量空状态

#### Scenario: 操作按钮
- **WHEN** `detail.actions` 包含 `create_work_order` 和 `ask_agent`
