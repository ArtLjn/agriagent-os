...
class GetDebtSummarySkill(Skill):
    """查询普通赊账欠款统计。"""
    ...
    def description(self) -> str:
        return (
            "查询欠款统计，支持普通赊账以及总欠款。"
            "用户问我还欠多少钱、我欠别人多少钱时用 scope=total_payable，"
            "汇总普通赊账和未付人工；用户明确问赊账还欠多少时用 debt_only。"
            "只问人工工资欠款时使用 get_labor_payables。"
        )
    ...
    def parameters_schema(self) -> dict:
        return {
            "type": "object",
            "properties": {
                "counterparty": {"type": "string", "description": "往来对象名称"},
                "direction": {
                    "type": "string",
                    "description": (
                        "payable=我欠别人，receivable=别人欠我，all=全部；默认 payable"
                    ),
                    "enum": [_PAYABLE, _RECEIVABLE, _ALL],
                    "default": _PAYABLE,
                },
                ...
                "scope": {
                    "type": "string",
                    "description": (
                        "debt_only=仅普通赊账，total_payable=普通赊账+未付人工；"
                        "默认 debt_only"
                    ),
                    "enum": [_DEBT_ONLY, _TOTAL_PAYABLE],
                    "default": _DEBT_ONLY,
                },
            },
        }
        ...
        try:
            ...
            if not summary:
                return SkillResult(
                    ...
                    reply=_empty_reply(direction, counterparty),
                    data={"summary": [], "total_remaining": "0.00"},
                )
            summary = summary[:limit]
            return SkillResult(
                status=ResultStatus.SUCCESS,
                reply=_format_reply(summary, direction),
                data={
                    "summary": summary,
            ...
            return SkillResult(
                status=ResultStatus.FAILED, reply=f"查询赊账欠款失败：{exc}"
            )
        finally:
            db.close()
...
def _summarize_records(records: list[CostRecord]) -> list[dict]:
...
def _format_reply(summary: list[dict], direction: str) -> str:
    total_remaining = _sum_remaining(summary)
    title = "应收赊账汇总" if direction == _RECEIVABLE else "赊账欠款汇总"
    lines = [f"{title}：总剩余{_money(total_remaining)}元"]
    for row in summary:
        lines.append(
...
def _format_total_payable_reply(
    debt_summary: list[dict],
    labor_workers: list[dict],
    *,
    debt_remaining: Decimal,
    labor_remaining: Decimal,
) -> str:
    total_remaining = debt_remaining + labor_remaining
    lines = [
        f"您目前总欠款为 {_money(total_remaining)} 元，构成如下：",
...
def _empty_reply(direction: str, counterparty: str | None) -> str:
...
def _sum_remaining(summary: list[dict]) -> Decimal:
    return sum(
        (_to_decimal(row["remaining"]) for row in summary),
        Decimal("0.00"),
    )
