...
type: write
description: 记录一笔农场支出或收入，支持自然语言记账和结构化赊账。
triggers:
  - 记账
  - 记一笔
  - 花了
...
parameters:
  ...
  properties:
    ...
    note:
      type: string
      description: "备注，如买苹果种子，向王秉着赊账。"
    record_subtype:
      type: string
      description: "记录子类型。赊账、欠账、未付款或未收款时传 赊账。"
    counterparty:
      type: string
      description: "赊账对象或未收款对象，如王秉着、老王农资店。"
    due_date:
      type: string
...
- “在农资店老王那赊了 3000 块大棚膜” -> `amount=3000`, `category=大棚膜`, `record_type=cost`, `record_subtype=赊账`, `counterparty=农资店老王`, `note=赊了大棚膜`。
- “卖辣椒给老李 5000，先欠着” -> `amount=5000`, `category=辣椒销售`, `record_type=income`, `record_subtype=赊账`, `counterparty=老李`。
- 赊账方向必须保守判断：买入/采购/花费赊账表示我方应付；卖出/销售未收款表示对方欠我方。只有“张三赊账”“欠张三”这类方向不完整表述时，不要断言谁欠谁，应先追问“是你欠张三，还是张三欠你？”。
...
- 用户表达“向张三赊账”且不是还款/结清语义时，按新增赊账记录处理；方向不完整的“张三赊账”“欠张三 2000”需要先追问欠款方向。
- 用户表达“还了张三”“结清张三”时不要使用本 Skill，应使用 `settle_debt`。
