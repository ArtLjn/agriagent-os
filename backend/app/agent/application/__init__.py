"""旧 `app.agent.application` 入口兼容层。

真实业务 use case 已迁移到 `app.application`。这里保留到 D4 迁移完成前，用于
兼容旧动态 import 与 monkeypatch target，并确保旧路径解析到同一模块对象。
"""
...
_NEW_PREFIX = "app.application"


class _ApplicationAliasLoader(importlib.abc.Loader):
...
class _ApplicationAliasFinder(importlib.abc.MetaPathFinder):
    def find_spec(
        self,
        fullname: str,
        path: object | None,
        ...
        spec = importlib.machinery.ModuleSpec(
            fullname,
            _ApplicationAliasLoader(fullname, target_name),
...
def _install_alias_finder() -> None:
    for finder in sys.meta_path:
        if isinstance(finder, _ApplicationAliasFinder):
            return
    sys.meta_path.insert(0, _ApplicationAliasFinder())
