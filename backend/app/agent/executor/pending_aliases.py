...
def pending_alias_metadata(skill_name: str) -> dict:
    """解析历史 pending action 的 legacy skill name，缺失时 fail closed。"""
    metadata = resolve_skill_capability_metadata(skill_name)
    if metadata is None:
    ...
    return {
        "legacy_tool_name": metadata["legacy_alias"],
...
def pending_alias_metadata_or_trace(skill_name: str, params: dict) -> dict:
    ...
    try:
        return pending_alias_metadata(skill_name)
