...
WRITE_PATTERNS: dict[str, list[re.Pattern]] = {
    ...
    "manage_crop_cycle": [
        ...
        re.compile(
            r"(?:到[了]?|进入)\s*(?:苗期|开花期|结果期|采收期|伸蔓期|定植期|膨大期)"
        ),
    ],
    "manage_crop_templates": [
        re.compile(r"(?:创建|建|新建|添加).*(?:作物|模[板版])"),
        re.compile(r"(?:没有|缺少|找不到).*(?:模[板版]|作物)"),
        re.compile(
            r"(?:修改|更改|调整|更新|改|删除|删掉|移除).*(?:作物模[板版]|模[板版])"
        ),
        re.compile(
            r"(?:作物模[板版]|模[板版]).*#?\d+.*(?:修改|更改|调整|更新|改|删除|删掉|移除)"
        ),
    ],
    "manage_farm_logs": [
