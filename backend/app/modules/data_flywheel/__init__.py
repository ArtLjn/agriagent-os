"""旧 `app.modules.data_flywheel` 入口兼容层。

真实 DataFlywheel 平台代码已迁移到 `app.platforms.data_flywheel`。这里仅保留
旧动态 import 与 monkeypatch target 的兼容，并确保旧路径解析到同一模块对象。
"""
...
_NEW_PREFIX = "app.platforms.data_flywheel"


class _DataFlywheelAliasLoader(importlib.abc.Loader):
...
class _DataFlywheelAliasFinder(importlib.abc.MetaPathFinder):
    def find_spec(
        self,
        fullname: str,
        path: object | None,
        ...
        spec = importlib.machinery.ModuleSpec(
            fullname,
            _DataFlywheelAliasLoader(fullname, target_name),
...
def _install_alias_finder() -> None:
    for finder in sys.meta_path:
        if isinstance(finder, _DataFlywheelAliasFinder):
            return
    sys.meta_path.insert(0, _DataFlywheelAliasFinder())
