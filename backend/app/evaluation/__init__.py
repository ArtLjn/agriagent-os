"""旧 `app.evaluation` 入口兼容层。

真实 Evaluation 平台代码已迁移到 `app.platforms.evaluation`。这里仅保留
...
_NEW_PREFIX = "app.platforms.evaluation"


class _EvaluationAliasLoader(importlib.abc.Loader):
...
class _EvaluationAliasFinder(importlib.abc.MetaPathFinder):
    def find_spec(
        self,
        fullname: str,
        path: object | None,
        ...
        spec = importlib.machinery.ModuleSpec(
            fullname,
            _EvaluationAliasLoader(fullname, target_name),
...
def _install_alias_finder() -> None:
    for finder in sys.meta_path:
        if isinstance(finder, _EvaluationAliasFinder):
            return
    sys.meta_path.insert(0, _EvaluationAliasFinder())
