...
async def test_collect_weather_candidates_uses_user_setting_coordinates(
    ...
    farm = Farm(id=77, name="定位农场", user_id=user.id, location="苏州")
    setting = UserSetting(
        user_id=user.id,
        default_city="徐州",
        default_lat=34.26,
...
async def test_collect_weather_candidates_falls_back_to_farm_location(
    db_session,
    monkeypatch,
):
    today = date(2026, 6, 12)
    farm = Farm(id=78, name="城市农场", location="南京")
    db_session.add(farm)
    db_session.commit()
    fetch_weather = AsyncMock(return_value=_weather_data(temps=[36, 32, 31]))
    monkeypatch.setattr(
