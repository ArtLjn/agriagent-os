...
class TestGetSettings:
    ...
    def test_returns_custom_display_name(self):
        """PUT 设置 display_name 后返回自定义值。"""
        response = client.put("/settings", json={"display_name": "老李"})

        assert response.status_code == 200
        assert response.json()["display_name"] == "老李"
...
class TestPutSettings:
    ...
    def test_update_persists_across_requests(self):
        """PUT 返回更新后的值。"""
        response = client.put("/settings", json={"display_name": "老张"})
    ...
    def test_update_overwrites_previous_value(self):
        ...
        response = client.put("/settings", json={"display_name": "老王"})
    ...
    def test_missing_field_keeps_existing(self):
        """缺少 display_name 字段时保持原有昵称。"""
        ...
        assert response.status_code == 200
        assert response.json()["display_name"] == "测试用户"
