...
def test_create_crop_template_intent_routes_to_write_tool() -> None:
    ...
    assert decision.selected_tools == ["create_crop_template"]
...
@pytest.mark.parametrize("message", ["有哪些作物模板", "模板列表", "我的茬口"])
...
def test_create_crop_template_with_read_entity_still_routes_to_write_tool() -> None:
    tools = [_tool("get_crop_templates"), _tool("create_crop_template")]
