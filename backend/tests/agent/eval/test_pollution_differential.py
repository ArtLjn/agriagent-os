...
class TestPollutionDifferentialE2:
    ...
    def test_select_tools_is_deterministic_for_query_intent(
        self, case, fake_tool_factory
    ):
        ...
        assert not result_clean.force_binding
...
class TestPollutionDifferentialBQuery:
    ...
    def test_query_selection_independent_of_context(self, case, fake_tool_factory):
        if not case.expected_skill:
            pytest.skip(f"{case.case_id}: no expected_skill")
