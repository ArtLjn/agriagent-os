...
| **碎片化** | 43 个 ≤30 行的微型文件、`stream_chat_*` 5 文件切片、`planner/`+`planning/` 双死目录 |
...
| **删除冗余依赖** | 移除 LangGraph；评估兼容层（compat.py）是否仍必要 |
...
3. **优先级：删除 > 合并 > 拆分 > 新建**：本阶段不新增任何抽象、skill 或子模块
...
| **死代码** | `planner/`（63 行）、`planning/`（160 行） | 223 |
...
| 步 | 动作 | 风险 |
| --- | --- | --- |
| C1 | `git mv app/agent/skills app/skills` | 低 |
| C2 | 全局搜替换 `from app.agent.skills` → `from app.skills` | 低 |
...
| 步 | 动作 | 风险 |
| --- | --- | --- |
| D1 | `git mv app/agent/application app/application` | 低 |
| D2 | 全局搜替换 `from app.agent.application` → `from app.application` | 低 |
...
| E4 | 删除 `pyproject.toml` 中 langgraph 与 langchain-core 依赖 | 低 |
...
| P2-3 | `document_repository_*` 砍掉 12 个未用 backend 类 | diagnosis 9-#2 |
...
P0 ──→ P1 ──→ P2 ──→ P3
        ...
        └─→ P2-3（删 12 个 backend 类）必须先于 P2-2 中 data_flywheel 整体迁移
```
