...
Agent 平台是 Farm Manager 的核心，由顶层 `application/` use case、`agent/` runtime 子域和若干工程化支撑域组成。以下路径按 2026-07-30 代码现状校准：
...
| **Runtime** | `agent/runtime/` | 纯 Python ReAct loop、节点、消息、状态、tool_executor、pending 拦截、LLM 调用、反思接入 | Prompt 版本治理、Context selector、Memory 存储、Planner 事实存储 |
| **Router** | `agent/router/` | 意图识别、候选检索、Skill Catalog、RouterPolicy、决策 trace | HTTP 路由、Prompt 模板、Memory 存储 |
| **Executor** | `agent/executor/` | Skill 调用、并行执行、参数校验、Pending Action、tool_calls 适配 | API 路由、Prompt 模板、Memory 存储、PlanIR 业务规划 |
...
| **Planning** | `agent/runtime/planning/`、`agent/runtime/plan_ir_pending.py`、`agent/task_graph/`、`agent/task_types.py` | PlanDraft adapter、PlanIR → ExecutionPlan → PendingPlan 投影、registry resolver、TaskType 统一；`task_graph/runtime` 仅作 legacy compile artifact | HTTP 路由、长期调度、独立 DAG Runtime |
...
```
请求入口
  ├─ application/chat 初始化会话、trace、pending 状态
  ├─ application/chat 读取 TaskState 并通过 Relevance Gate 判断是否注入
  ├─ agent/guardrails 检查输入与写操作计划
  ├─ agent/router 基于当前输入和相关 TaskState 生成候选工具和 RouterDecision
  ├─ context/builder 根据 task_state_should_inject 构建 ContextBundle
  ├─ agent/runtime/loop.py 执行 ReAct loop
  ├─ agent/runtime/planning 将 PlanIR 编译为 ExecutionPlan 并投影 PendingPlan
...
| `planning/` | `PlanDraft`/`PlanIR` 到 Runtime pending 结构的 adapter、模型与 validator |
| `plan_ir_pending.py` | 从 `AgentState.plan_ir` 构建 `PendingPlan` candidate，非法计划 fail-closed |
| `tool_pending.py` | pending plan / pending action 拦截入口，优先处理 PlanIR candidate，再保留 runtime draft fallback |
...
```
PlanIR 或 LLM tool_call 表达写意图
  ↓
若是 PlanIR：先校验并编译为 ExecutionPlan
  ↓
ExecutionPlan 按 registry resolver 解析 capability / operation → skill_name / params
  ↓
多步骤写入 → 创建 PendingPlan（带 steps、TTL、metadata、trace）
  ↓
单个 tool_call → Executor 检查 skill.meta.permission == "write_confirm"
  ↓
是 → 创建 PendingAction（带完整参数 + 过期时间）
     → 返回确认话术
     → 用户确认 → pending action / pending plan executor 真正执行 Skill
     → 用户取消 → 下一轮 /agent/chat 消息进入 pending handler → 丢弃
     → 超时 → 异步清理
否 → 直接执行 Skill
```

PlanIR 是规划表达，不拥有执行权。`ExecutionPlan` 是 Runtime 面向执行的中间层，负责把 `PlanIRStep(capability, operation, inputs)` 收敛为 `ExecutionStep(skill_name, params, requires_confirmation, side_effect)`；任何 malformed PlanIR、未知 operation、参数契约不满足或危险 side effect 都必须阻断并写 trace。
...
| `agent/runtime/planning/` | PlanIR adapter、ExecutionPlan、PendingPlan candidate、registry resolver | 独立任务调度、直接写业务库、替代 Executor |
| `agent/task_graph/` | PlanIR 模型、compiler、task-specific planner artifact、离线评测 | 在线执行 Runtime；`task_graph/runtime` 只保留 legacy 标记 |
...
| Planning | 保留 Runtime B；吸收 Planning A 的计划表达能力，通过 `PlanIR → ExecutionPlan → PendingPlan` adapter 接入，不恢复双任务系统 |
| 上下文压缩 | `context/compressors` + token budget + cache 三层协作 |
| TaskState | 作为跨轮工作任务状态；读取后先经过相关性门，低相关历史任务不影响 Router 和 Context |
...
| Final 回复 | Final Agent 独立于 Tool Agent；最终回复阶段必须使用 `tools=[]`、`tool_choice=none`，并通过 FinalContextBuilder 隔离原始 `AIMessage(tool_calls)` 历史 |
...
- [16_Agent日志与诊断设计](./16_Agent日志与诊断设计.md)
- [03_接口协议/02_Agent内部接口](../03_接口协议/02_Agent内部接口.md)
- [../../design/agent-plan-mechanisms.md](../../design/agent-plan-mechanisms.md)
- [../../specs/2026-07-30-agent-task-planning-runtime-convergence.md](../../specs/2026-07-30-agent-task-planning-runtime-convergence.md)
