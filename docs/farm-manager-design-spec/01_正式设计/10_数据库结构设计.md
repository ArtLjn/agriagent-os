...
本文档定义 AgriAgentOS 后端 MySQL 8.x 的**完整表结构设计**。它从接口协议（HTTP API / Agent 内部接口 / 外部服务接口 / Skill 契约）反推实体与字段，落地到具体表。
...
冲突时以代码为准。ORM 模型由 `backend/app/shared/model_registry.py` 汇总导入，真实模型分布在 `agent/`、`context/`、`domains/`、`memory/`、`platforms/`、`shared/` 等目录；代码改动必须同步本文档。

## 2. 设计原则

| 原则 | 落地方式 |
| --- | --- |
| **多租户隔离** | 所有业务表带 `farm_id` 外键，外层用 `domains/farm/dependencies.get_current_farm` 注入；跨 `farm_id` 查询在 [03_数据库与迁移规范 § 5.2](../04_相关规范/03_数据库与迁移规范.md) 明确禁止 |
...
| **农事与用工** | 7 | `farm_logs`、`farm_log_workers`、`planting_units`、`operation_work_orders`、`operation_work_order_units`、`workers`、`labor_entries` |
| **会话与 Agent** | 7 | `conversations`、`conversation_messages`、`agent_turns`、`agent_records`、`agent_pending_plans`、`agent_pending_plan_steps`、`agent_task_states` |
| **记忆** | 1 | `memory_records` |
| **数据飞轮** | 5 | `agent_data_flywheel_labels`、`agent_data_flywheel_prelabels`、`agent_case_drafts`、`agent_repair_packs`、`agent_review_issue_chains` |
| **仿真评测** | 2 | `simulation_runs`、`simulation_results` |
| **可观测** | 4 | `trace_records`、`token_daily_stats`、`guardrails_logs`、`feedback_records` |
| **基础设施** | 3 | `idempotency_keys`、`mongo_compensation_tasks`、`alembic_version`（Alembic 自维护） |
| **仍未落地** | 3 | `audit_logs`、`evaluation_reports`、`skill_call_stats` |
...
erDiagram
    ...
    farm_logs ||--o{ farm_log_workers : "farm_log_id"
    workers ||--o{ farm_log_workers : "worker_id"
    operation_work_orders ||--o{ farm_logs : "work_order_id"
...
**地域化策略**：当前代码的系统模板列表为 `GET /crops/templates/system?category=`；`region_tag` / `dedup_key` 仍是 openspec 提案维度，未成为当前路由参数。
...
**关联接口**：`GET/POST /cycles`、`DELETE /cycles/{id}`。
...
**关联接口**：`GET/POST/DELETE /costs`、`GET /costs/summary/{year}`、`GET /costs/cycles/{cycle_id}/profit`、`GET /debts`（filter `record_subtype='赊账' AND settlement_status<>'settled'`）。
...
- 还款/收款不新增经营账单。`POST /debts/settle` 与 `settle_debt` 只更新原赊账记录的 `settled_amount`、`settlement_status`、`settled_at`。
...
| `work_order_id` | `INT` | FK→operation_work_orders, NULL | 来源作业单，仅用于追溯 |
...
**业务边界**：`farm_logs` 是农事事实账，只记录浇水、打药、施肥、除草、翻地等操作事实。它可以关联来源作业单和参与人用于追溯，但不直接生成工资、不结算人工，也不作为发薪日应付金额的统计来源。

### 8.1.1 `farm_log_workers` — 农事日志参与人

| 字段 | 类型 | 约束 | 说明 |
| --- | --- | --- | --- |
| `id` | `INT` | PK | — |
| `farm_log_id` | `INT` | FK→farm_logs, CASCADE | 农事日志 |
| `worker_id` | `INT` | FK→workers | 参与工人 |
| `role` | `VARCHAR(50)` | NULL | 参与角色，预留 |
| `note` | `VARCHAR(500)` | NULL | 备注 |

**唯一约束**：`uq_farm_log_workers_log_worker (farm_log_id, worker_id)`。

**业务边界**：`farm_log_workers` 只用于展示“谁参与了这条农事事实”，不创建、不更新 `labor_entries`，也不参与应发工资计算。
...
**关联接口**：`GET/POST /planting/work-orders`。

**业务边界**：`operation_work_orders` 是“作业 + 范围 + 用工”的桥接账。创建或更新作业单时，如果携带工人和计薪字段，可同步生成 `labor_entries`；如果只记录作业事实，则不应自动推断工资。作业单可作为农事日志和工资明细的共同来源引用。
...
**关联接口**：`GET /planting/labor/unsettled-summary`、`GET /planting/workers/summary`。

**业务边界**：`labor_entries` 是发薪统计的唯一主账。发薪日按 `worker_id`、`work_date`、`quantity`、`unit_price`、`payable_amount`、`paid_amount` 和 `unpaid_amount` 汇总每个工人应该发多少钱。农事日志缺失不会影响已登记工资的统计；如果既没有农事日志也没有工资明细，系统不能凭空推断工资，只能提示补录出勤或工资记录。
...
**关联接口**：`GET /agent/conversations`、`GET /agent/conversations/{session_id}/messages`。
...
**关联接口**：当前未暴露独立 `/pending/*` HTTP 路由；确认/取消由 `/agent/chat` 相关交互和 pending plan 服务承载。
...
**关联接口**：`POST /simulation/run`、`GET /simulation/runs`、`GET /simulation/reports/{run_id}`。
...
**关联接口**：`GET /admin/traces`、`GET /admin/traces/requests`、`GET /admin/traces/{request_id}/timeline`。
...
**关联接口**：`GET /admin/stats/tokens`、`GET /admin/stats/tokens/daily`、`GET /admin/stats/tokens/hourly`。
...
**关联接口**：`POST /agent/feedback`、`GET /agent/feedback/stats`。
...
**用途**：Smart Fill `POST /smart-fill/parse` 防重复解析；`cleanup_old_keys(hours=24)` 定时清理。
**关联接口**：`GET /smart-fill/scenarios`、`POST /smart-fill/parse`。
...
**当前最新 revision**：`20260722_explicit_memory_records`（以 `cd backend && alembic heads` 为准）。

## 14. 模型与迁移差异对账

当前仓库不再保留 `backend/sql/farm_manager.sql` 生产 dump。对账基准改为 ORM 模型注册表（`backend/app/shared/model_registry.py`）+ Alembic 迁移（`backend/alembic/versions/`）+ 实际部署库 `alembic current`。

| 项 | 当前代码状态 | 处置 |
| --- | --- | --- |
| `memory_records` | 已由 `memory/long_term/models.py` 与 revision `20260722_explicit_memory_records` 落地 | 不再列为预留待建 |
| `agent_review_issue_chains` | 已由 `platforms/data_flywheel/models.py` 与相关迁移落地 | 问题链不是待建项 |
| `agent_task_states` | 已由 `context/task_state_models.py` 与 revision `20260722_agent_task_states` 落地 | 用于任务状态上下文 |
| `mongo_compensation_tasks` | 已由 `shared/mongo_compensation_models.py` 与 revision `20260626_mongo_compensation_tasks` 落地 | 用于 Mongo 双写失败补偿 |
| `crop_templates.region_tag` / `dedup_key` | 代码与迁移已包含系统模板库基础、分类和去重索引；地域化 delta 仍以实际模型为准 | 若继续演进地域化，需重新核对 `crop_models.py` 与 Alembic |
| `evaluation_reports` | 尚未见 ORM 表 | Evaluation 报告当前由平台代码生成模型/文件或聚合，不作为已落地表描述 |

**对账流程**：每次发布前，开发环境跑 `alembic upgrade head` → `alembic current` / `alembic heads` → 用 `shared/model_registry.py` 的 `Base.metadata` 与实际库结构比对 → 在本节登记。
...
| `evaluation_reports` | 评测汇总报告 | 需要持久化历史报告时 | [06_数据飞轮 § 8 P3](./06_数据飞轮与评测.md) |
...
`memory_records` 已落地，核心字段以 `backend/app/memory/long_term/models.py` 为准：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | `INT` PK | 自增内部 ID |
| `memory_id` | `VARCHAR(64)` UNIQUE | 对外稳定 ID |
| `farm_id` | `INT` FK→farms, INDEX | — |
| `user_id` | `VARCHAR(64)` INDEX | 用户 ID |
| `type` | `VARCHAR(32)` INDEX | `preference`/`habit`/`alias`/`event`/`fact` 等 |
| `content` | `TEXT` | 自然语言内容 |
| `status` | `VARCHAR(20)` | `candidate`/`confirmed`/`superseded`/`archived` |
| `source` | `VARCHAR(20)` | `user_explicit`/`llm_extracted` |
| `importance` / `confidence` | `FLOAT` | 重要性与置信度 |
| `superseded_by_id` | `VARCHAR(64)` | 软覆盖指针 |
| `created_at` / `updated_at` / `archived_at` | `DATETIME` | 生命周期字段 |
...
| HTTP 端点 | 主操作表 | 只读关联 |
| --- | --- | --- |
| `POST /auth/login` / `register` | `users`（校验/创建） | — |
...
| `GET /agent/daily` / `POST /agent/daily/refresh` | `agent_records`（record_type=daily） | `crop_cycles`, `cost_records`, `farm_logs` |
| `GET /agent/conversations` / `conversations/{session_id}/messages` | — | `conversations`, `conversation_messages` |
| 对话确认 / pending plan 执行 | `agent_pending_plans`, `agent_pending_plan_steps` | 各业务表（通过 Skill 执行） |
| `GET/POST /smart-fill/scenarios` / `smart-fill/parse` | `idempotency_keys` | Smart Fill 编排各业务表 |
...
| `GET/POST /logs` | `farm_logs`, `farm_log_workers` | `crop_cycles`, `operation_work_orders`, `workers` |
| `GET/POST /planting/work-orders` | `operation_work_orders`, `operation_work_order_units`, `labor_entries` | `planting_units`, `workers` |
| `GET /planting/workers` | — | `workers` |
| `GET /planting/labor/unsettled-summary` | — | `labor_entries`, `workers` |
| `GET /weather/*` | — | 外部 QWeather/Open-Meteo，无落盘 |
| `GET/PUT /settings` | `user_settings` | — |
| `POST /agent/feedback` | `feedback_records` | — |
| `GET /admin/users` | — | `users` |
| `GET /admin/skills` | — | SkillRegistry（运行时扫描 `skills/*/skill.md`，不入库） |
| `GET /admin/traces` / `traces/{request_id}/timeline` | — | `trace_records` 或 Mongo `traceRecords` |
| `GET /admin/data-flywheel/samples` | — | `agent_turns` LEFT JOIN `agent_data_flywheel_labels`/`prelabels` |
| `POST /admin/data-flywheel/samples/{sample_id}/labels` | `agent_data_flywheel_labels` | — |
| `POST /simulation/run` | `simulation_runs`, `simulation_results` | `agent_case_drafts`（用例源） |
| `GET /admin/stats/tokens` | — | `token_daily_stats` |
...
| `MemoryService.consolidate` | `conversations.summary` + `memory_records` |
| `TraceCollector.emit` | `trace_records` + JSONL `logs/traces/{date}/{trace_id}.jsonl` |

## 17. 索引策略汇总

按 [03_数据库与迁移规范 § 2.4](../04_相关规范/03_数据库与迁移规范.md) 的索引规范，关键复合索引以 ORM 和 Alembic 为准：
...
**注意**：索引清单需以 Alembic 与实际库为准；旧生产 dump 已不在仓库中，不再作为索引事实源。

Discovery Layer 已落地到 `platforms/evaluation/discovery/`，新增风险字段或索引时必须同步本节和对应 Alembic revision。
...
- [04_Memory工程 § 7.2](./04_Memory工程.md) — `memory_records` 长期记忆设计
...
- 代码事实来源：[backend/app/shared/model_registry.py](../../../backend/app/shared/model_registry.py)、[backend/alembic/versions/](../../../backend/alembic/versions/)
