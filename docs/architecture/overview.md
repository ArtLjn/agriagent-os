...
当前后端是 FastAPI + SQLAlchemy + LangGraph + Skill 工具调用体系。入口已迁移到 `app/bootstrap/`，Agent 平台已拆出 application、runtime、planner、executor、response、context、memory、platforms/evaluation、observability 等边界；迁移期只保留已登记且有调用证据的旧技术层入口。
...
`platforms/data_flywheel` 已承接 Agent 数据飞轮的 admin router、样本服务、问题链服务和 repair pack 闭环；其中 `review_issue_chain/` 与 `repair_pack/` 是问题链和 repair pack 子域真实入口，DataFlywheel root 下的问题链、repair pack、judge service 与 repository selector 兼容薄壳已下线。`platforms/evaluation` 已承接 Evaluation 真实代码，旧 Evaluation 根包已下线；`platforms/shared` 当前包含 judge service 与 repository selector 的真实入口。`api/` 不再保留 DataFlywheel 的平铺路由入口。`modules/crop`、`modules/cycle`、`modules/ledger`、`modules/weather`、`modules/conversation`、`modules/feedback`、`modules/admin` 属于后续迁移目标。除明确记录的兼容入口外，当前不保留只有 `__init__.py` 的空壳目录，避免目录树和真实职责脱节。
